...
yourmom-own[
  nag     ; if this is true it means yourmom is in the basic searching  mode
  clean   ; if this is true it means yourmom is heading back to base
  trash   ; if this is true it means yourmom is heading for a resource
  pile    ; if this is true it means yourmom has found a cluster and is counting rocks
  rockLocations
  momx
  momy
]
...
to setup
 ...
 birth-yourmom ;creates robot called your mom
...
;------------------------------------------------------------------------------------
 ...
 ;------------------------------------------------------------------------------------
to birth-yourmom
  create-yourmom number-of-yourmoms [
    set size 6
    set shape "robot"
    set color pink
    set heading 360
  ; set label "YourMom"
  ; pen-down
    set nag true           ;starts out searching for rocks
    set clean false
    set trash false
    set pile false
    set movement 7
...
to robot-control
 ...
 ;------------------------------------------------------------------------------------
 ;;;;;;;;;;;;;;;;;;;;;;;;;
 ;;yourmom robot-control;;  robot control procedure for your mom
 ;;;;;;;;;;;;;;;;;;;;;;;;;
 ;------------------------------------------------------------------------------------
  ...
  ask yourmom[
    ...
    if nag   [nagging-mom] ;runs the code nagging-mom(search for rocks) if nag is set to  true
    if clean [mom-go-home] ;runs the code mom-go-home(returning to base) if clean is set to true
    nosy-mom
    if pile [motherload]
 ...
 ;------------------------------------------------------------------------------------
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;;       yourmom     ;;  this begins the code for the robot named your mom.
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;------------------------------------------------------------------------------------
 ;------------------------------------------------------------------------------------
 ...
 ;------------------------------------------------------------------------------------
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;;   nosy-mom        ;;  this is the basic movement code for your mom
 ;;;;;;;;;;;;;;;;;;;;;;;  CHANGE IT IF YOU WANT ITS NOT IMPORTANT
 ;------------------------------------------------------------------------------------
to nosy-mom
  if (clean = false) and (trash = false) and (pile = false) [
    right random angle
  left random angle
  ]
 ...
 ;------------------------------------------------------------------------------------
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;;    nagging-mom    ;;   this is the search for rocks procedure
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;------------------------------------------------------------------------------------
to nagging-mom
  ask yourmom[
    ...
    ;only does it if there is a rock within range
    if (any? patches with [pcolor = yellow] in-radius 2) [


        ;this part of the code sets a target for yourmom if there are any rocks
        ;in a radius of 2 and puts it in targeting mode (trash)

            set patchx ( [pxcor] of one-of patches with [pcolor = yellow] in-radius 2)
            set patchy ( [pycor] of one-of patches with [pcolor = yellow] in-radius 2)
            ask yourmom[
              set trash true
;            ]
;          ]
    ...
    ;says that unless it reaches the rock it should face the rock
    ask self[
       
      if (trash = true)[
        ifelse (any? yourmom-on patches with [pcolor = yellow])[
           if count patches in-radius 2 with [pcolor = yellow] >= clustercount [ ;if there are more than 1 rocks
     ;set momx [pxcor] of patch-ahead 7 
     ;set momy [pycor] of patch-ahead 7
      motherload]
        ...
        if (pile = false) [
          ...
          ask yourmom [                      ;} this part sets the robot to pick up mode
            set shape "robot with rock"      ;}sets shape
            set nag false                    ;}stops searching
            set trash false                  ;}stops aiming for a spot
            set clean true                   ;}makes it aim to the base
 ...
 ;------------------------------------------------------------------------------------
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;;    Motherload     ;;  this is when searching a cluster
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;------------------------------------------------------------------------------------
to motherload
 ...
 ;------------------------------------------------------------------------------------
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;;    Mom-go-home    ;;  this is when your mom is going home
 ;;;;;;;;;;;;;;;;;;;;;;;
 ;------------------------------------------------------------------------------------
to mom-go-home
  ...
  [
    facexy 0 0     ;faces the base if it is not there yet
  ]
